...
const cellSize = 33.3; // size in %
const lengthArr = 8; // 4x4
const rows = 3; // количество рядов и строк
const className = 'thri_numbers'; // klass
...
function move(index) {
  ...
  const isFinish = arrCells.every((cell) => {
    console.log('val', cell.value, 'top', cell.top, cell.left);
...
// shuffle arr
function shuffle() {
  return (shuffleArr = [...Array(lengthArr).keys()]);
